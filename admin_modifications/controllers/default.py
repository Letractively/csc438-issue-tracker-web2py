...
def newissue():
    ...
    default_summary='Ticket: ' + str(ticket)
    default_description= str(e.output)
    
    form=SQLFORM.factory(
        Field('project_id', 'string', requires=IS_NOT_EMPTY()),
        Field('summary', 'string', default=default_summary, requires=IS_NOT_EMPTY()),
        Field('description', 'text', default=default_description, requires=IS_NOT_EMPTY()),
        Field('owner', 'string', requires=IS_NOT_EMPTY())
    )
    
    """handle submission"""
    if form.process().accepted:   
        import xmlrpclib
        server=xmlrpclib.ServerProxy(issuetracker_host)
        
        """set variables from form"""
        summary=form.vars.summary
        description=form.vars.description
        owner=form.vars.owner
        projectid=form.vars.project_id

        """attempt to submit new issue to remote tracker"""
        try:
            result=server.newissue(projectid, summary, description, owner)
            result=str(result)
            session.flash='New Issue Submitted. Issue Number: ' + result 
            redirect(URL('errors',args=[app]))
            #return dict(form=form)
        except xmlrpclib.Fault, err:
            result = 'an error occurred '
            result += str(err.faultCode)
            result += err.faultString
            response.flash=str(result)
            return dict(form=form)
    elif form.errors:
